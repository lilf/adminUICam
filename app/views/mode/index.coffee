...
module.exports = class ColumnNode extends Backbone.Node
  ...
  logCreate: (model) ->
    @domainPub 'log', 'create', 'mode', model.id, model.toJSON()

  logUpadte: (model) ->
    @domainPub 'log', 'update', 'mode', model.id, model.toJSON()

  logDestroy: (model) ->
    @domainPub 'log', 'destroy', 'mode', model.id, model.toJSON()
