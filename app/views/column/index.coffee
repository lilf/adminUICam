...
module.exports = class ColumnNode extends Backbone.Node
  ...
  initialize: ->
    ...
    $('#column').html @list_view.render().el
  ...
  logCreate: (model) ->
    @domainPub 'log', 'create', 'column', model.id, model.toJSON()

  logUpadte: (model) ->
    @domainPub 'log', 'update', 'column', model.id, model.toJSON()

  logDestroy: (model) ->
    @domainPub 'log', 'destroy', 'column', model.id, model.toJSON()
